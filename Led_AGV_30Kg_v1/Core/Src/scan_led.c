...
/*
NOTE: Code nay su dung cho mach Led AGV 30Kg
...
/*_____________________________________________________________________*/
	...
	void led24(Status_e status){
		if(status == on)
			HAL_GPIO_WritePin(LED24_GPIO_Port, LED24_Pin, GPIO_PIN_RESET);
		else
			HAL_GPIO_WritePin(LED24_GPIO_Port, LED24_Pin, GPIO_PIN_SET);
	}
	
	void led25(Status_e status){
		if(status == on)
			HAL_GPIO_WritePin(LED25_GPIO_Port, LED25_Pin, GPIO_PIN_RESET);
		else
			HAL_GPIO_WritePin(LED25_GPIO_Port, LED25_Pin, GPIO_PIN_SET);
	}
	
	void led26(Status_e status){
		if(status == on)
			HAL_GPIO_WritePin(LED26_GPIO_Port, LED26_Pin, GPIO_PIN_RESET);
		else
			HAL_GPIO_WritePin(LED26_GPIO_Port, LED26_Pin, GPIO_PIN_SET);
	}
	
	void led27(Status_e status){
		if(status == on)
			HAL_GPIO_WritePin(LED27_GPIO_Port, LED27_Pin, GPIO_PIN_RESET);
		else
			HAL_GPIO_WritePin(LED27_GPIO_Port, LED27_Pin, GPIO_PIN_SET);
	}
...
/*_____________________________________________________________________*/	
	...
	{ 	
		...
		choose_led(27 - pFL->cur_led_haiben +1, on);
...
/*_____________________________________________________________________*/	
	void loading(flag_irq_t *pFL)
	{
		...
		for(uint8_t cnt = 27; cnt >= pFL->cur_led_loading; cnt--){
			choose_led(cnt, on);
		}
	
		pFL->cur_led_loading--;
		if(pFL->cur_led_loading == 0)
			pFL->cur_led_loading = 28;
		...
		switch(pFL->MODE)
		{
			...
			case SNAKE:
			{
				if(++pFL->snake >= 20){
					pFL->snake = 0;
					snake(pFL);
					if(++pFL->cnt_snake >= 93){			// thuc hien 3 lan: = (soled+4)x3
						pFL->cnt_snake = 0;
						Full_LED(off);
						pFL->MODE = RUNNING;
					}
