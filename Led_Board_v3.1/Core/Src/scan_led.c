...
/*_____________________________________________________________________*/		
	void nhayled(void){ 	// nhay toan bo led
...
/*_____________________________________________________________________*/		
	void nhaymotphanled(uint8_t type){ 	// nhay mot phan led
		static bool sts_1p = true;
		
		switch (type){
			//---------
			case DB_L:
				for(uint8_t iR=12;iR<=23;iR++)	choose_led(iR, off);
				if(sts_1p){
					for(uint8_t iL=1;iL<=12;iL++)	choose_led(iL, on);	
					sts_1p = false;
				}
				else{
					for(uint8_t iL=1;iL<=12;iL++)	choose_led(iL, off);	
					sts_1p = true;
				}
				break;
			//---------
			case DB_R:
				for(uint8_t iL=1;iL<=12;iL++)	choose_led(iL, off);	
				if(sts_1p){
					for(uint8_t iR=12;iR<=23;iR++)	choose_led(iR, on);
					sts_1p = false;
				}
				else{
					for(uint8_t iR=12;iR<=23;iR++)	choose_led(iR, off);
