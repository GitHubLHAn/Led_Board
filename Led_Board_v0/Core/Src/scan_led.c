...
/*_____________________________________________________________________*/	
	void hai_ben(flag_irq_t *pFL){ 	
		...
		choose_led(18 - pFL->cur_led_haiben, on);
...
/*_____________________________________________________________________*/	
	void loading(flag_irq_t *pFL){
		...
		for(uint8_t cnt = 17; cnt >= pFL->cur_led_loading; cnt--){
			choose_led(cnt, on);
		}
	...
	void HANDLE_MODE(flag_irq_t *pFL){
		...
		if(pFL->MODE == HAI_BEN){
			if(++pFL->haiBen >= 30){
				pFL->haiBen = 0;
				hai_ben(pFL);
				if(++pFL->cnt_haiBen >= 56){			// thuc hien 3 lan
					pFL->cnt_haiBen = 0;
					pFL->MODE = LOADING;
				}
			}
		...
		//--------------------------------------------------------
			...
			if(++pFL->snake >= 30){
				pFL->snake = 0;
				snake(pFL);
				if(++pFL->cnt_snake >= 58){			// thuc hien 3 lan
					pFL->cnt_snake = 0;
					Full_LED(off);
					pFL->MODE = RUNNING;
				}
