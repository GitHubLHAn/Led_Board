...
/*_____________________________________________________________________*/
	void HANDLE_MODE(flag_irq_t *pFL){
		...
		//--------------------------------------------------------
		else if(pFL->MODE == FULL_ON){
			Full_LED(on);
		}
		//--------------------------------------------------------
		else if(pFL->MODE == RUNNING){
			static bool d = true;
			...
			if(d){
				if(++pFL->running >= 15   ){
					...
					//choose_led(cnt_led_runningMode, off);
					//HAL_GPIO_WritePin(LED12_GPIO_Port, LED12_Pin, GPIO_PIN_SET);
					//HAL_GPIO_TogglePin(LED1_GPIO_Port, LED1_Pin);
					d = false;
					
					if(--cnt_led_runningMode == 4) 
						cnt_led_runningMode = 23;	//23		
				}
//					if(++pFL->haiBen >= 30){
//						pFL->haiBen = 0;
//						hai_ben(pFL);
//						if(++pFL->cnt_haiBen >= 26){			// thuc hien 1 lan
//							pFL->cnt_haiBen = 0;
//							d = false;
//						}
//					}
				...
				if(++pFL->running >= 1000){
					...
					choose_led(cnt_led_runningMode, on);
					choose_led(cnt_led_runningMode - 1, on);
					choose_led(cnt_led_runningMode - 2, on);
					choose_led(cnt_led_runningMode - 3, on);
					choose_led(cnt_led_runningMode - 4, on);
					//HAL_GPIO_WritePin(LED12_GPIO_Port, LED12_Pin, GPIO_PIN_RESET);
					//HAL_GPIO_TogglePin(LED1_GPIO_Port, LED1_Pin);
